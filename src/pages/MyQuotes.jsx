...
export default function MyQuotes() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-24 pt-[calc(1.5rem+env(safe-area-inset-top))]">
      ...
      <h1 className="mb-5 text-2xl font-bold tracking-tight text-neutral-900 dark:text-neutral-100">Your Own Quotes</h1>

      <div className="mb-6 rounded-2xl border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
        <textarea value={newText} onChange={(e) => setNewText(e.target.value)} placeholder="Write your own motivational quote..." rows={3} className="w-full resize-none text-sm text-neutral-900 outline-none dark:bg-neutral-900 dark:text-neutral-100" />
      ...
      {quotes.length === 0 ? (
        <p className="py-12 text-center text-sm text-neutral-400 dark:text-neutral-500">No quotes yet. Add your first one above!</p>
      ) : (
        <div className="space-y-3">
          {quotes.map((q) => (
            <div key={q.id} onClick={() => navigate(`/quote/${q.id}`)} className="flex cursor-pointer items-start gap-3 rounded-2xl border border-neutral-200 bg-white p-4 transition-colors hover:bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-900 dark:hover:bg-neutral-800">
              <p className="select-text flex-1 text-sm text-neutral-800 dark:text-neutral-200">{q.text}</p>
              <button onClick={(e) => { e.stopPropagation(); handleDelete(q.id); }} className="shrink-0 text-neutral-300 hover:text-red-400 dark:text-neutral-600">
                <Trash2 size={16} />
              </button>
            </div>
          ))}
